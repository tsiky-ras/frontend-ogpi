...
import { CreateBacklogRequest } from "../../../../types/lead/Backlog/Backlog.tsx";
...
const BacklogForm: React.FC<BacklogFormProps> = ({ show, onClose, onSubmit, leadId }) => {
  ...
  const [form, setForm] = useState<CreateBacklogRequest>({
    name: "",
    leadId: leadId,
    desc: "",   // on utilise "desc" pour correspondre au backend
  });
  ...
  const handleSubmit = async () => {
    ...
    try {
      await onSubmit({
        name: form.name.trim(),
        leadId: form.leadId,
        desc: form.desc.trim(), // envoi de la description
      });
      setForm({ name: "", leadId, desc: "" });
      onClose();
    } catch (err: any) {
          ...
          <Row className="g-3">
            ...
            <Col md={12}>
              ...
              <Form.Control
                as="textarea"
                rows={3}
                name="desc"         // doit correspondre au state
                value={form.desc}
                onChange={handleChange}
