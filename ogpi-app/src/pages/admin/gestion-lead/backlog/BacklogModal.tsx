...
const BacklogModal: React.FC<BacklogModalProps> = ({ show, onClose, leadId, leadName }) => {
  ...
  // États existants pour le backlog sélectionné
  ...
  // Set of phase ids that are expanded (show deliverables)
  ...
  const fetchBacklogsList = async () => {
    try {
      ...
      // Si un seul backlog, le sélectionner automatiquement
      if (fetchedBacklogs.length === 1) {
        setSelectedBacklogId(fetchedBacklogs[0].id);
      }
    } catch (err) {
  ...
  const fetchBacklog = async () => {
    ...
    try {
      ...
      // Charger (précharger) les livrables pour chaque phase afin que le badge
      // affiche le bon nombre avant le clic de l'utilisateur.
      ...
      for (const phase of allPhases) {
        try {
          ...
          console.log(
            `Livrables chargés pour la phase ${phase.id} (${phase.name}):`,
            phaseDeliverables
          );
          const sortedDeliverables = [...(phaseDeliverables || [])].sort(
  ...
  // Deliverable drag-and-drop has been disabled per request.
  // The previous Sortable initialization for deliverables was removed to prevent dragging.
  ...
  const togglePhase = async (phaseId: number) => {
    ...
    // if deliverables already loaded, just expand
    if (deliverables.has(phaseId)) {
      setExpandedPhases(prev => new Set(prev).add(phaseId));
      return;
    }

    // load deliverables on demand
    ...
    try {
      ...
      const sortedDeliverables = [...(phaseDeliverables || [])].sort((a, b) => (a.order ?? 0) - (b.order ?? 0));
      setDeliverables(prev => {
        const next = new Map(prev);
        next.set(phaseId, sortedDeliverables);
        return next;
      });
      setExpandedPhases(prev => new Set(prev).add(phaseId));
    } catch (err) {
      console.error(`Erreur lors du chargement des livrables pour la phase ${phaseId}:`, err);
    ...
    try {
      if (editingDeliverable) {
        const updatedDeliverable = await backlogDeliverableService.update(editingDeliverable.id, {
          ...
          phaseId: currentPhaseId ?? editingDeliverable.phaseId,
        });
    ...
    return (
      <Modal show={show} onHide={onClose} size="xl" fullscreen>
        <Modal.Header closeButton>
          <Modal.Title>Backlogs - {leadName || 'Chargement...'}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <div
          ...
          >
            ...
            <span>Chargement des backlogs...</span>
          </div>
        </Modal.Body>
      </Modal>
    );
          ...
          <div className="container-fluid">
            ...
            >
                        ...
                        {backlogs.map((backlogItem) => (
                          <div key={backlogItem.id} className="col-md-6 col-lg-4 mb-3">
                            <div 
                              ...
                              onClick={() => setSelectedBacklogId(backlogItem.id)}
                            >
                              <div className="card-body">
                                <h5 className="card-title">{backlogItem.name}</h5>
                                {backlogItem.desc && (
                                  <p className="card-text text-muted small">{backlogItem.desc}</p>
                                )}
                                <div className="d-flex justify-content-end mt-3">
                                  <small className="text-primary">
                                    <FaEye className="me-1" /> Cliquez pour voir les détails
                                  </small>
                                </div>
                              </div>
                            </div>
              ...
              <Tab eventKey="lots" title="Lots et Phases">
                {!selectedBacklogId ? (
                  <div className="text-center text-muted py-5">
                    <p>Veuillez sélectionner un backlog pour gérer les lots et phases.</p>
                  </div>
                ) : (
                  <>
                    <div className="d-flex justify-content-end mb-3">
                                            ...
                                            <div className="d-flex justify-content-between align-items-center mb-2">
                                              ...
                                              <div className="phase-actions">
                                                <button
                                                  ...
                                                  onClick={(e) => { e.stopPropagation(); openAddDeliverable(phase.id); }}
                                                ...
                                                <button
                                                  className="btn-icon"
                                                  onClick={(e) => { e.stopPropagation(); openEditPhase(phase, lot.id); }}
                                                  title="Modifier"
                                                >
                                                  <FaEdit />
                                                </button>
                                                <button
                                                  className="btn-icon"
                                                  onClick={(e) => { e.stopPropagation(); deletePhase(phase.id, lot.id); }}
                                                  title="Supprimer"
                                                >
                                                  <FaTrash />
                                                </button>
                                                          ...
                                                          <div className="deliverable-content">
                                                            <div className="d-flex justify-content-between align-items-start">
                                                              ...
                                                              <div className="deliverable-actions">
                                                                <button
                                                                  className="btn-icon"
                                                                  onClick={(e) => { e.stopPropagation(); openEditDeliverable(deliverable, phase.id); }}
                                                                >
                                                                  <FaEdit />
                                                                </button>
                                                                <button
                                                                  className="btn-icon"
                                                                  onClick={(e) => { e.stopPropagation(); deleteDeliverable(deliverable.id, phase.id); }}
                                                                >
                                                                  <FaTrash />
                                                                </button>
                                                              </div>
              ...
              <Tab eventKey="profils" title="Profils">
                {!selectedBacklogId ? (
                  <div className="text-center text-muted py-5">
                    <p>Veuillez sélectionner un backlog pour gérer les profils.</p>
                  </div>
                ) : (
                  <>
                    <div className="d-flex justify-content-end mb-3">
